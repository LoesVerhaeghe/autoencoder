...
# Function to display original and reconstructed images
def visualize_reconstruction_and_SVI(model, device, test_loader, num_images=5, path='outputs'):
    model.eval()  # Set the model to evaluation mode
    with torch.no_grad():  # Disable gradient calculation for testing/inference
        for image_batch, SVI_measurements in test_loader:
            image_batch, SVI_measurements = image_batch.to(device), SVI_measurements.to(device)
            reconstructed, SVI_pred = model(image_batch)
            ...
            for i in range(num_images):
                # Original images
                axes[0, i].imshow(image_batch[i].permute(1, 2, 0).numpy(), cmap='grey')  # Convert CHW to HWC
                axes[0, i].set_title('Original')
                axes[0, i].axis('off')
